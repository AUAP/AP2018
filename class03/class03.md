...
- conditional statments: can put in the && and || in this notes to tell the differences in relation to sequence and how machine reads if statments.
...
- So for first time programming learners, arrays and for loop are more difficult concepts. I think it is because it suddenly changes the spatial and temporal dimensions, in terms of the extension of the conept of variables where more values can be incorporated. Additionally, the loop is also related to time and process: the happening of each loop and how to handle the loop and what's the limitation of that loop. 
- Alan Turing's halting problem would make sense to talk in the AP book in this chapter, in relation to loop and wait for the uncertain/unknowable. (loop forever, strange loop, infinite loop, etc. Some materials from PhD last chapter on automated agents..) 
- the mini ex seems slightly off track, i should bring it back to think about time and temporality through a throbber. next time...
